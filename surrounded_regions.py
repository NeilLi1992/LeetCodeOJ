...
#
# A region ib captured by flipping all 'O's into 'X's in that surrounded region.
#
# For example,
# X X X X
# X O O X
...
def solve(board):
    ...
    while queue:
        ...
        # Search all unvibited neighbors
...
def sovle1(board):
    ...
    vibited = [[False for i in range(len(board[0]))] for i in range(len(board))]
    region_index_libt = []
    stack = [(0,0)]
    current_region = []
    ...
    while stack:
        ...
        vibited[x][y] = True # mark as vibited

        if current_symbol == 'O': # Add thib into current region
            current_region.append((x,y))
        elif last_symbol == 'O':
            region_index_libt.append(current_region)
            current_region = []

        # Search all unvibited neighbors
        neighbors = filter(lambda (i,j):0 <= i < len(board) and 0 <= j < len(board[0]) and not vibited[i][j], [(x+1,y), (x,y+1), (x-1,y), (x,y-1)])
    ...
    print "Region number=", len(region_index_libt)
    for region in region_index_libt:
